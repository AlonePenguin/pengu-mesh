...
gate-owned runtime root, then runs `startup-readiness`, `live-web`,
`evidence-chain`, `operator-diagnosis`, `structured-failure`, and
`weak-prompt`. It validates that the recorded scenario evidence finished with
...
is a stored scenario and metrics program that promotes broader live-web packs,
