...
| --- | --- | --- | --- | --- |
| active-next | Metrics database and scenario recorder (in progress) | the storage foundation, aggregate summary, and first multi-family scenario evidence gate are now real, but the repo still needs broader threshold- and comparison-grade use of recorded data | `crates/pengu-mesh-state/src/lib.rs`, `crates/pengu-mesh-shared/src/types.rs`, `crates/pengu-mesh-core/src/lib.rs`, `docs/observability.md`, `docs/performance-budget.md` | scenario storage is exercised by multiple named families, scenario-list/detail/summary/gate remain truthful, and stored metrics drive more gate or budget decisions |
...
- the next serious lane expands scenario gates beyond the current startup-readiness/evidence-chain/operator-diagnosis/structured-failure/weak-prompt manifest into repeated live-web, fresh-agent, and PinchTab comparison proof
...
- ownership and capability gating move from visible posture and preflight hints
  to authenticated executable contract
