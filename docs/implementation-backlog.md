...
| high | Authenticated ownership and capability gating | trusted-local `holder_id` is still not enough for a stronger product claim | `crates/pengu-mesh-state/src/lib.rs`, `crates/pengu-mesh-core/src/lib.rs`, `crates/pengu-mesh-shared/src/types.rs`, `docs/security-model.md` | real ownership model, risk tiers, and typed denial paths across CLI/MCP/HTTP |
...
- ownership and capability gating move from docs-only to executable contract
