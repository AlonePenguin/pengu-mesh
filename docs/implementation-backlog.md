...
| --- | --- | --- | --- | --- |
| active-next | Metrics database and scenario recorder (in progress) | the storage foundation and first aggregate summary are now real, but the repo still needs threshold- and comparison-grade use of the recorded data | `crates/pengu-mesh-state/src/lib.rs`, `crates/pengu-mesh-shared/src/types.rs`, `crates/pengu-mesh-core/src/lib.rs`, `docs/observability.md`, `docs/performance-budget.md` | scenario storage is exercised by multiple named families, scenario-list/detail/summary remain truthful, and the stored metrics start driving gate or budget decisions |
...
- the next serious lane turns the stored scenario summary into thresholded or comparative proof instead of read-only reporting
