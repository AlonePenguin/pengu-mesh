...
| high | Authenticated ownership and capability gating | the safe/elevated/dangerous posture is now visible in health, doctor, and the dashboard, but trusted-local `holder_id` is still not enough for a stronger product claim | `crates/pengu-mesh-state/src/lib.rs`, `crates/pengu-mesh-core/src/lib.rs`, `crates/pengu-mesh-shared/src/types.rs`, `docs/security-model.md`, `web/dashboard/` | real ownership model and enforced typed denial paths across CLI/MCP/HTTP that use the surfaced risk tiers |
